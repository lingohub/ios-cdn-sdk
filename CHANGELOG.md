...
- Failed update checks follow one retry and pause policy, shared with the Android SDK and documented in the README's new "Failures and retries" section (lingohub/organization#2351):
  ...
  - An update check that is running when the app changes the environment or the CDN key stops at its next step without changes: no further request (not even its retry), no install, and nothing recorded over what the new configuration recorded. The update then checks with the new configuration and reports that result.
